...
- `vibegraph login` and `vibegraph key change` use device authorization. `vibegraph mcp install` writes a local IDE configuration that invokes `vibegraph mcp-proxy`, so IDEs never receive a raw secret in a workspace file.
...
- `/mcp` is never an unauthenticated direct integration: clients use the CLI stdio proxy or send
  `X-API-Key` from a secret/environment store.
