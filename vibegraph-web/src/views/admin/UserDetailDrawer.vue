...
// F-M6 split: the logic lives in user-detail-format.ts (pure, unit-tested); these thin
// wrappers only bind the drawer's i18n `t`/locale so the template stays untouched.
const usedMb = fmt.usedMb
const quotaMb = fmt.quotaMb
const storagePercent = fmt.storagePercent
const userStatus = fmt.userStatus

function userStatusLabel(u: AdminUserResponse): string {
  return fmt.userStatusLabel(t, u)
}
    ...
    <div class="drawer">
      ...
      <div ref="drawerBodyRef" class="drawer-body">
        ...
        <UserApiKeyList
          ...
          @lock="handleLockApiKey"
          @unlock="handleUnlockApiKey"
        />
      </div>
    </div>
