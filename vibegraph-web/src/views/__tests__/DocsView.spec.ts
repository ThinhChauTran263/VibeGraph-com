...
describe('DocsView', () => {
  ...
  it('documents the production CLI and MCP commands', () => {
    ...
    expect(text).toContain('vibegraph key status')
    expect(text).toContain('vibegraph key clear')
    ...
    expect(text).not.toContain('vibegraph auth status')
    expect(text).not.toContain('vibegraph auth clear')
    expect(text).not.toContain('vibegraph projects push')
  ...
  it('documents the published npm package and production install command', () => {
    ...
    expect(text).toContain('vibegraph-cli@0.1.7')
    expect(text).toContain('npm install -g vibegraph-cli')
  })
