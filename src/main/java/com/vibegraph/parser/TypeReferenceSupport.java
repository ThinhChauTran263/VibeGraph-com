...
public final class TypeReferenceSupport {
    ...
    private static Optional<String> resolveImportedType(String importedName) {
        ...
        return validateAgainstProjectRegistry(importedName);
    }
    ...
    private static Optional<String> resolveSimpleType(String simpleName, Node context) {
        ...
        if (simpleName.contains(".")) {
            return hasSkippedPrefix(simpleName) ? Optional.empty() : validateAgainstProjectRegistry(simpleName);
        }

        Optional<String> candidate = context.findCompilationUnit()
                ...
                .or(() -> {
                    return context.findCompilationUnit()
                            ...
                            .filter(resolvedType -> !shouldSkipImportedType(resolvedType));
                })
                .or(() -> Optional.of(simpleName));
        return candidate.flatMap(TypeReferenceSupport::validateAgainstProjectRegistry);
    }
    ...
    private static Optional<String> validateAgainstProjectRegistry(String candidate) {
        if (candidate == null || candidate.isBlank()) {
            return Optional.empty();
        }
        return ProjectSymbolRegistry.current()
                .map(registry -> registry.contains(candidate) ? Optional.of(candidate) : Optional.<String>empty())
                .orElseGet(() -> Optional.of(candidate));
    }
